...
const one =
  '<div class="container"><div class="row"><div class="col-md-6"><div class="column-content"><div class="benefits-container"><div class="desc-title type-gamma">Benefits</div><div class="benefits-list">';
const two =
  '</div></div><div class="features-container"><div class="desc-title type-gamma">Features</div><div class="features-list">';
const three =
  '</div></div></div></div><div class="col-md-6"><div class="column-content"><div class="ingredients-container"><div class="ingredients-header"><div class="type-gamma">Key ingredients</div></div>';
const four =
  '<div class="ingredients-footer"><div class="type-epsilon text-medium mt-1"><a data-toggle="modal" data-target="#modalProductInfo-05" href="#">Show all ingredients</a></div></div></div></div></div></div></div>';
const five =
  '<div class="modal fade" id="modalProductInfo-05" tabindex="-1" role="dialog" aria-label="myModalLabel" aria-hidden="true"> <div class="modal-dialog modal-lg"> <div class="modal-content"> <div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span class="icon icon-clear"></span> </button> </div><div class="modal-body">';
...
class App extends React.Component {
  ...
  addDescription = FeatureThatHasBeenAddedInFeaturesFormInputFieldAndLabelIncluded => {
    ...
    newFeature[
      `feature${Date.now()}`
    ] = FeatureThatHasBeenAddedInFeaturesFormInputFieldAndLabelIncluded;
    //3. Set the new state
    this.setState({
  ...
  addBenefit = BenefitThatHasBeenAddedInBenefitsCheckboxInput => {
    ...
    newBenefit[
      `benefit${Date.now()}`
    ] = BenefitThatHasBeenAddedInBenefitsCheckboxInput;
    this.setState({
      benefits: newBenefit
    });
  };
  ...
  addKeyIngredient = IngredientThatHasBeenAddedInIngredientsFormInputFields => {
    ...
    newIngredient[
      `ingredient${Date.now()}`
    ] = IngredientThatHasBeenAddedInIngredientsFormInputFields;
    //3. Set the new state
    this.setState({
  ...
  addAllIngredients = AllIngredientsThatHasBeenAddedInAllIngredientsFormInputField => {
    ...
    newIngredient[
      `allingredients1`
    ] = AllIngredientsThatHasBeenAddedInAllIngredientsFormInputField;
    //3. Set the new state
    this.setState({
    ...
    return (
      <div className="App">
        <div className="container">
              ...
              <div className="output-features">
                ...
                {Object.keys(this.state.features).map(key => (
                  <OutputFeatures
                    key={key}
                    index={key}
                    featureprop={this.state.features[key]}
                    deleteF={this.deleteFeature}
                  />
                ))}
              </div>
            </div>
              ...
              <div className="output-benefits">
                ...
                {Object.keys(this.state.benefits).map(key => (
                  <OutputBenefits
                    key={key}
                    index={key}
                    benefitprop={this.state.benefits[key]}
                    deleteB={this.deleteBenefit}
                  />
                ))}
              </div>
            </div>
          ...
          <div className="row section">
            <div className="col-6">
              <div className="key-ingredients">
                <KeyIngredients addKeyIngredient={this.addKeyIngredient} />
              </div>
            </div>
            <div className="col-6">
              {/* Whenever there is something in state, we are going to map over and for each render this: */}
              {Object.keys(this.state.keyingredients).map(key => (
                <OutputKeyIngredients
                  key={key}
                  index={key}
                  keyingredientsprop={this.state.keyingredients[key]}
                  deleteI={this.deleteKeyIngredient}
                />
              ))}
            </div>
          </div>
          <div className="row">
            <div className="col-12">
              <div className="title">All Ingredients</div>
            </div>
          </div>
          <div className="row section">
            <div className="col-6">
            ...
            <div className="col-6">
              <div className="output-all-ingredients">
                ...
                {Object.keys(this.state.allingredients).map(key => (
                  <OutputAllIngredients
                    key={key}
                    index={key}
                    allingredientsprop={
                      this.state.allingredients.allingredients1
                    }
                  />
                ))}
              </div>
